...
#Read in the data
filename <- "data/light_dark_rna_fpkms_CPM_DEG_18june19_count.csv" #file containing the read count matrix
library(ggbiplot)
...
#drop M37W
#@countsNonZero <- countsNonZero[,-grep("M37W",colnames(countsNonZero) )]
#colnames(countsNonZero)
...
write.csv(coldata, "covariates_for_PEER.csv")


write.csv(model.matrix(~ -1 +treatment, coldata), "treatment_covariates_for_PEER.csv", row.names =F)
y <- t(countsNonZero)
x <- prcomp(y, scale. = T)
