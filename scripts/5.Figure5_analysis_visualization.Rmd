...
ggsave("Figure_5a_supplement.png",width = 24, height =12)

```

Figure S14 code chunk:
input: figure5b_data.csv
data: 2020 Cornell Greenhouse experiment 
Tissue type: Mature Kernels
...
#format data
re <- re[,-c(1:3)]
colnames(re)[1:2] <- c("geno", "Genotype")
traits <- colnames(re)[3:12]
results_list <- list()
anova_list <- list()
tukey_list <- list()
#Perform Tukey's HSD and check for outliers
...
for ( i in 1: length(traits)){
  x <-  unlist(as.vector( re[,traits[i]]))
  print(traits[i])
  df <- data.frame("TT" = x, "geno" = re$geno)
...
re_m <- melt(as.data.table(re), id.vars = c("geno","Genotype"))
colnames(re_m)[3:4] <- c("name","value")
re_m$name <- replace_with_greek(re_m$name)
max_values <- aggregate(value ~ name, data = re_m, FUN = max)
...
re_m <- re_m %>% filter(!(name %in% c("Total Tocochromanol", "Total.Carots")))



re_m$name <- factor(re_m$name, levels = c( "α-Tocotrienol" ,"δ-Tocotrienol","γ-Tocotrienol", "Total Tocotrienols", "α-Tocopherol" , "δ-Tocopherol" ,"γ-Tocopherol",   "Total Tocopherols"))
tukey_df$name <- factor(tukey_df$name, levels = c( "α-Tocotrienol" ,"δ-Tocotrienol","γ-Tocotrienol", "Total Tocotrienols", "α-Tocopherol" , "δ-Tocopherol" ,"γ-Tocopherol",     "Total Tocopherols"))
...
ggsave("Figure_5b_supplemental.png",width = 24, height =12)
